...
function ProductEditor({ product, catalog, onCancel, onSave }: { product: Product | null; catalog: PublicCatalog; onCancel: () => void; onSave: (product: Product) => Promise<void> }) {
  const [form, setForm] = useState<Product>(product ?? { id: crypto.randomUUID(), categoria_id: catalog.categories[0]?.id ?? "", nombre: "", descripcion: "", imagen_url: "", precio: 0, precio_anterior: null, disponible: true, recomendado: false, nuevo: false, activo: true, orden: catalog.products.length + 1 });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  function change<K extends keyof Product>(key: K, value: Product[K]) { setForm((current) => ({ ...current, [key]: value })); }
  async function upload(file?: File) { if (!file) return; try { change("imagen_url", await uploadProductImage(file)); } catch { setError("No se pudo subir la imagen."); } }
  ...
  return <section className="admin-card"><div className="admin-card-header"><div><h2>{product ? "Editar producto" : "Nuevo producto"}</h2><p>Los cambios se reflejan en el catálogo público</p></div></div><form className="admin-form" onSubmit={submit}><label className="image-upload span-2"><Upload size={22} /><strong>{form.imagen_url ? "Cambiar fotografía" : "Subir fotografía"}</strong><small>JPG, PNG o WebP. Se guarda en Supabase Storage.</small><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => upload(event.target.files?.[0])} /></label><label className="field"><span>Nombre *</span><input value={form.nombre} onChange={(event) => change("nombre", event.target.value)} /></label><label className="field"><span>Categoría *</span><select value={form.categoria_id} onChange={(event) => change("categoria_id", event.target.value)}>{catalog.categories.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></label><label className="field span-2"><span>Descripción *</span><textarea rows={3} value={form.descripcion} onChange={(event) => change("descripcion", event.target.value)} /></label><label className="field"><span>Precio *</span><input type="number" min="1" step="0.01" value={form.precio} onChange={(event) => change("precio", Number(event.target.value))} /></label><label className="field"><span>Precio anterior</span><input type="number" min="0" step="0.01" value={form.precio_anterior ?? ""} onChange={(event) => change("precio_anterior", event.target.value ? Number(event.target.value) : null)} /></label><label className="field span-2"><span>URL de imagen *</span><input value={form.imagen_url} onChange={(event) => change("imagen_url", event.target.value)} placeholder="https://…" /></label><label className="switch-field"><span>Disponible</span><input className="switch" type="checkbox" checked={form.disponible} onChange={(event) => change("disponible", event.target.checked)} /></label><label className="switch-field"><span>Recomendado</span><input className="switch" type="checkbox" checked={form.recomendado} onChange={(event) => change("recomendado", event.target.checked)} /></label><label className="switch-field"><span>Nuevo</span><input className="switch" type="checkbox" checked={form.nuevo} onChange={(event) => change("nuevo", event.target.checked)} /></label><label className="switch-field"><span>Activo</span><input className="switch" type="checkbox" checked={form.activo} onChange={(event) => change("activo", event.target.checked)} /></label>{error && <div className="form-error span-2">{error}</div>}<div className="form-actions"><button className="button button-secondary button-small" type="button" onClick={onCancel}>Cancelar</button><button className="button button-primary button-small" type="submit" disabled={saving}>{saving ? "Guardando…" : "Guardar producto"}</button></div></form></section>;
}
...
function ZonesSection({ catalog, setCatalog }: { catalog: PublicCatalog; setCatalog: React.Dispatch<React.SetStateAction<PublicCatalog | null>> }) {
  ...
  return <div className="settings-columns"><section className="admin-card"><div className="admin-card-header"><div><h2>Zonas configuradas</h2><p>El costo se suma al pedido</p></div></div><div className="simple-list">{catalog.zones.map((item) => <div key={item.id}><span className="list-index"><MapPinned size={15} /></span><div><strong>{item.nombre} · {formatCurrency(item.costo, catalog.settings.simbolo_moneda)}</strong><small>{item.tiempo_estimado} · Mínimo {formatCurrency(item.pedido_minimo ?? 0, catalog.settings.simbolo_moneda)}</small></div><button className={`status-badge ${item.activa ? "entregado" : "cancelado"}`} type="button" onClick={() => toggle(item)}>{item.activa ? "Activa" : "Inactiva"}</button></div>)}</div></section><section className="admin-card"><div className="admin-card-header"><div><h2>Nueva zona</h2><p>Define costo, tiempo y pedido mínimo</p></div></div><form className="admin-form" onSubmit={add}><label className="field span-2"><span>Nombre</span><input value={form.nombre} onChange={(event) => setForm({ ...form, nombre: event.target.value })} /></label><label className="field"><span>Costo</span><input type="number" min="0" value={form.costo} onChange={(event) => setForm({ ...form, costo: Number(event.target.value) })} /></label><label className="field"><span>Pedido mínimo</span><input type="number" min="0" value={form.pedido_minimo} onChange={(event) => setForm({ ...form, pedido_minimo: Number(event.target.value) })} /></label><label className="field span-2"><span>Tiempo estimado</span><input value={form.tiempo_estimado} onChange={(event) => setForm({ ...form, tiempo_estimado: event.target.value })} /></label><div className="form-actions"><button className="button button-primary button-small" type="submit"><Plus size={15} /> Añadir zona</button></div></form></section></div>;
}
...
function PaymentsSection({ catalog, setCatalog }: { catalog: PublicCatalog; setCatalog: React.Dispatch<React.SetStateAction<PublicCatalog | null>> }) {
  const [name, setName] = useState("");
  async function add(event: FormEvent) { event.preventDefault(); if (!name.trim()) return; const item: PaymentMethod = { id: crypto.randomUUID(), nombre: name.trim(), descripcion: "", activo: true }; await saveCatalogRecord("paymentMethods", "metodos_pago", item); setCatalog((current) => current ? { ...current, paymentMethods: [...current.paymentMethods, item] } : current); setName(""); }
  ...
  return <div className="settings-columns"><section className="admin-card"><div className="admin-card-header"><div><h2>Métodos disponibles</h2><p>El cliente solo verá los activos</p></div></div><div className="simple-list">{catalog.paymentMethods.map((item) => <div key={item.id}><span className="list-index"><CreditCard size={15} /></span><div><strong>{item.nombre}</strong><small>{item.descripcion || "Sin descripción"}</small></div><button className={`status-badge ${item.activo ? "entregado" : "cancelado"}`} type="button" onClick={() => toggle(item)}>{item.activo ? "Activo" : "Inactivo"}</button></div>)}</div></section><section className="admin-card"><div className="admin-card-header"><div><h2>Nuevo método</h2><p>Configura otra forma de pago</p></div></div><form className="admin-form" onSubmit={add}><label className="field span-2"><span>Nombre</span><input value={name} onChange={(event) => setName(event.target.value)} placeholder="Ej.: Transfermóvil" /></label><div className="form-actions"><button className="button button-primary button-small" type="submit"><Plus size={15} /> Añadir método</button></div></form></section></div>;
}
