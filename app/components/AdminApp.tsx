...
import { extrasFromOrder, formatCurrency, formatDate, orderStatusLabels } from "@/app/lib/format";
...
function ProductEditor({ product, catalog, onCancel, onSave }: { product: Product | null; catalog: PublicCatalog; onCancel: () => void; onSave: (product: Product) => Promise<void> }) {
  const [form, setForm] = useState<Product>(product ?? { id: crypto.randomUUID(), categoria_id: catalog.categories[0]?.id ?? "", nombre: "", descripcion: "", imagen_url: "", precio: 0, precio_anterior: null, extra_nombre: "", extra_costo: 0, disponible: true, recomendado: false, nuevo: false, activo: true, orden: catalog.products.length + 1 });
